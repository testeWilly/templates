...
function alteraFinalizar(categoria, metodo, idioma){
    ...
    var evento = "javascript:if (validaCampos() && !__postback) {__postback = true;__doPostBack('btnFinalizarCompra_" + categoria + "_" + metodo + "_" + idioma + "_" + parcelas + "',''); }";
        ...
        else if ($(this).attr('name') == 'txtNumCartao' || $(this).attr('name') == 'txtNumCartao_moip')
        {
                ...
                else
                {
                      ...
                      //Solução: possibilitar a digitação do número do cartão reestringindo a limitação de dígitos(14 a 16 números).
                        ...
                        if (($(this).attr('value').trim().length < quantidadeMaxima) && !($(this).attr('value').trim().length >= 14)  )
                        {
                            alert('O campo número do cartão deve ter entre 14 e ' + quantidadeMaxima + ' digitos.');
                            validado = false;
                            return validado;
                        }
                }
...
//        }
        else if($(this).attr('name') == 'txtCodigoSeguranca' || $(this).attr('name') == 'txtCodigoSeguranca_moip') 
        {
                ...
                if($(this).attr('value').trim() == '000')
                {
                          alert('Não preencha o código do cartão com zeros.'); 
                          validado = false;
                          return validado;
...
//Tarefa 87626 : Número do cartão e código de segurança estão aceitando mais digitos do que o permitido e também estão aceitando letras.
//Função: função para restringir a inserção de caracteres e bloquear a tecla shift.
function somenteNumeros(e) {
    ...
    //Teclas 48 a 57 e 96 a 105 são as teclas de números.
    //Tecla 46 é a tecla Delete.
      

    if (((tecla >= 48 && tecla <= 57) || (tecla >= 96 && tecla <= 105)) || (tecla == 8) || (tecla == 9) || (tecla == 37) || (tecla == 39) || (tecla == 36) || (tecla == 35) || (tecla == 46)) {
        return true;
    }
    else {
        return false;
